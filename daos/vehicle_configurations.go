...
func (dao *VehicleDAO) GetConfigurationDetails(rs app.RequestScope, vehicleid, deviceid int) (*models.VehicleConfigDetails, error) {
	var vdetails models.VehicleConfigDetails
	query := "SELECT conf_id, vc.device_id, vc.vehicle_id, vc.owner_id, fitter_id, vc.device_status, notification_email, notification_no, data FROM vehicle_configuration AS vc "
	...
	query += " LIMIT 1"
	q := rs.Tx().NewQuery(query)
	err := q.Row(&vdetails.ConfigID, &vdetails.DeviceID, &vdetails.VehicleID, &vdetails.OwnerID, &vdetails.FitterID, &vdetails.DeviceStatus, &vdetails.NotificationEmail, &vdetails.NotificationNO, &vdetails.Data)
	return &vdetails, err
}
