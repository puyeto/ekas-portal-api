...
func (dao *VehicleDAO) GetVehicleByStrID(rs app.RequestScope, strid string) (*models.VehicleConfigDetails, error) {
	...
	query += " fitter_id, notification_email, notification_no, COALESCE(JSON_VALUE(data, '$.device_detail.sim_no'), '') AS sim_no, last_seen, COALESCE(vr.renewal_date, vd.created_on) AS renewal_date, renew, "
...
func (dao *VehicleDAO) CheckIfVehicleIsExpired(rs app.RequestScope, cd *models.Vehicle, daystoexpiry int) error {
	...
	query += " FROM vehicle_details AS vd LEFT JOIN vehicle_renewals AS vr ON (vr.vehicle_id = vd.vehicle_id) WHERE vd.vehicle_string_id='" + vehiclestringid + "'"
