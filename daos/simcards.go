...
func (dao *SimcardDAO) Count(rs app.RequestScope) (int, error) {
	var count int
	err := rs.Tx().Select("COUNT(*)").From("simcards").Row(&count)
	return count, err
}

// Query retrieves the simcard records with the specified offset and limit from the database.
func (dao *SimcardDAO) Query(rs app.RequestScope, offset, limit int) ([]models.Simcards, error) {
	simcards := []models.Simcards{}
	err := rs.Tx().Select().OrderBy("identifier ASC").Offset(int64(offset)).Limit(int64(limit)).All(&simcards)
	return simcards, err
}
